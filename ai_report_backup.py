...
class BrainTumorAnalyzer:
    ...
    def analyze_prediction(self, metrics: Dict, mri_shape: tuple, pred_mask: np.ndarray, key_slices: Optional[Dict] = None) -> Generator[str, None, None]:
        tumor_locations = self._get_tumor_locations(pred_mask)
        severity = self._assess_severity(metrics, mri_shape)
        
        key_slices_info = ""
        if key_slices and key_slices.get('top_slices'):
            key_slices_info = f"\n\nKEY DIAGNOSTIC SLICES (Highest Tumor Content):"
            for i, slice_data in enumerate(key_slices['top_slices'][:3]):
                key_slices_info += f"\n- {slice_data['axis'].title()} slice {slice_data['slice_idx']}: {slice_data['tumor_percentage']:.1f}% tumor coverage ({slice_data['tumor_pixels']:,} pixels)"
...
- Regular screening if family history or risk factors present"""
        ...
        # Build the analysis string step by step to avoid f-string issues
        analysis_parts = []
        analysis_parts.append("**CLINICAL SUMMARY**")
        analysis_parts.append(f"Brain tumor segmentation analysis reveals {severity} with volume of {tumor_vol:,} voxels ({tumor_frac:.2f}% of total brain volume).")
        analysis_parts.append("")
        analysis_parts.append("**TUMOR CHARACTERISTICS**")
        analysis_parts.append(f"• Size Assessment: {severity} occupying {tumor_frac:.2f}% of brain volume")
        analysis_parts.append(f"• Volume Measurement: {tumor_ml} mL detected")
        analysis_parts.append(f"• Location: Identified in {tumor_locations}")
        analysis_parts.append("• Morphology: AI segmentation indicates defined tumor boundaries")
        analysis_parts.append("")
        analysis_parts.append("**RISK ASSESSMENT**")
